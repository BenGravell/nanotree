...
static constexpr Color COLOR_DARK = COLOR_GRAY_016;
static constexpr Color COLOR_LITE = COLOR_GRAY_240;

const Color COLOR_GOOD = guppyColor(0.15f);
...
const Color COLOR_OBJECT_BRUSH_FRGD = Fade(COLOR_GRAY_128, 0.6f);
...
Color computeFrameTimeColor(const float duration) {
    ...
    if (duration < 0.050f) return COLOR_FPS_MID;
