...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        num_nodes = std::clamp(num_nodes + ((GetMouseWheelMove() > 0) - (GetMouseWheelMove() < 0)) * 100, 0, 5000);
        ...
        DrawText("LMB: move goal  RMB: add obstacle  MMB: del obstacle  Scroll: max nodes", 10, 840, 20, RAYWHITE);
        DrawText((std::to_string(num_nodes) + " max nodes").c_str(), 140, 810, 20, BLUE);
        DrawFPS(10, 810);
        EndDrawing();
    }
    CloseWindow();
