...
const std::shared_ptr<Node> getNearest(const Vector2 target) {
    return *std::min_element(nodes.begin(), nodes.end(), [&target](const std::shared_ptr<Node>& a, const std::shared_ptr<Node>& b) { return Vector2Distance(a->pos, target) < Vector2Distance(b->pos, target); });
}

void DrawNode(const std::shared_ptr<Node> node, const float line_width, const Color color) {
    DrawLineEx(node->parent ? node->parent->pos : node->pos, node->pos, line_width, color);
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonDown(0) || IsMouseButtonDown(1) || IsMouseButtonDown(2)) {
            nodes.clear();
            nodes.push_back(std::make_shared<Node>(Node(start, nullptr)));
            for (int i = 0; i < 4000; ++i) {
                Vector2 pos = {800 * (float(std::rand()) / RAND_MAX), 800 * (float(std::rand()) / RAND_MAX)};
                std::shared_ptr<Node> parent = getNearest(pos);
            ...
            path.clear();
            std::shared_ptr<Node> node = getNearest(goal);
            while (node->parent) {
                path.push_back(node);
                node = node->parent;
            }
        ...
        for (const Vector2 obstacle : obstacles) DrawCircleV(obstacle, 40, DARKGRAY);
        for (const auto& node : nodes) DrawNode(node, 2, DARKBLUE);
        for (const auto& node : path) DrawNode(node, 8, RAYWHITE);
